...
'''
    ...
    cand_list,target_type "field",下所有target_id，即field_id
'''
...
def node_classification_sample(seed, pairs, time_range):
    '''
        ...
        pairs : {target_id:[[source_ids,...],time]}，target--paper, source--field
        time_range: 时间范围
    ...
    '''
        ...
        掩码”paper"-"field"-"PF_in_L2"边,target_ser,即paper的ser>batch_size则，添加入masked_edge_list;
        如此，则将采样的target节点(paper类节点)的所有边，即待预测的边，全部从新edge_list中删除；
        
    '''
    masked_edge_list = []
    for i in edge_list['paper']['field']['rev_PF_in_L2']:
        # i[0],target_ser,paper的ser
    ...
    # target_ids，为采样的batch_size个sel_train_pair，
    # 对于在rev_PF_in_L2下采样得到的每个paper节点，找出其所有相连的source节点(field);
    # 找到每个source节点(field)在PF_in_L2下的位置,令其值为1，
    # 进行归一化，即找到每个采样节点与每个连接的source节点(field)连接的概率；
    for x_id, target_id in enumerate(target_ids):
        # pairs[target_id][0]包含多个与target_id相连的source_id,target_id为paper的ID
        # source_id为field的id；
        for source_id in pairs[target_id][0]:
            # 从cand_list中搜寻source_id的index,然后令ylabel[x_id]中对应的index值为1
            # cand_list,field的id列表
            #
