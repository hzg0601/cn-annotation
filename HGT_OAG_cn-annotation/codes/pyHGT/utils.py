...
# CG（Cumulative Gain）表示累积增益,rel_i 表示查询结果列表中第 i 个位置的相关性值（相关程度可以理解为权重），
# 即该结果和查询有多大联系，可以看出CG不会考虑查询结果的顺序，即如果随机打乱查询结果， CG 的值也不会变。
# CG_k = \sum_{i=1}^k{rel_i}
# DCG(Discounted Cumulated Gain)_k,代表第 k 个位置的折扣累积增益，rel_i 表示第 i 个位置相关程度，其分子代表第 i 个位置结果的相关程度，
# 分母是位置 i 的对数表示。如果将相关程度高的结果排序在列表的后面，会导致 DCG 的值变低，
# 即惩罚了这种情况，所以以 DCG 作为目标的排序会使最相关的结果排列在最靠前的位置。
# 版本1 DCG_k = \sum_{1}^k{rel[i]/np.log2(i+1)} + rel[0]
# 版本2 DCG_k = \sum_{1}^k{(2^rel[i]-1)/np.log2(i+1)}
# NDCG(Normalized Discounted Cumulation Gain)_k = DCG_k / IDCG(Ideal Discounted Cumulated Gain)
# IDCG = \sum_{i=1}^{REL_k}{(rel[i])}/{log2(i+1)}
# REL_k表示语料库中相关性最高的 k 个文档列表(已按相关性排序)
def dcg_at_k(r, k):
    # np.as_float_array
    # 把r变为float array,然后取其前k个值
    r = np.asfarray(r)[:k]
    if r.size:
        # \sum_{1}^k{r[i]/np.log2(i)} + r[0]
...
def timeit_method(method):
    ...
    def wrapper(self,*args,**kwargs):
        ...
        oaglog.logger.info('运行耗时%s秒' % runtime)
        return f
