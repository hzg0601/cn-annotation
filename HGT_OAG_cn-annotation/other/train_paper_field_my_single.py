...
target_relation = [["paper", "field", "'rev_PF_in_L2'"], ['field', 'paper', 'PF_in_L2']]
'''
...
gnn = GNN(conv_name=args.conv_name,
          in_dim=len(graph.node_feature['paper']['emb'].values[0]) + 401 + np.sum(args.max_sprw) + 3,
          n_hid=args.n_hid,
...
for epoch in np.arange(args.n_epoch) + 1:
    ...
    '''
    oaglog.logger.info("第%s epoch训练开始。。。。" % str(epoch))
    train_data = [job.get() for job in jobs[:-1]]
    valid_data = jobs[-1].get()
    pool.close()
    pool.join()
    oaglog.logger.info("第%s epoch数据准备完毕。。")
    '''
    ...
    '''
    oaglog.logger.info("第%s epoch训练完成" % str(epoch))
    model.eval()
...
with torch.no_grad():
    ...
    oaglog.logger.info('Last Test NDCG: %.4f' % np.average(test_ndcg))
    test_mrr = mean_reciprocal_rank(test_res)
    oaglog.logger.info('Last Test MRR:  %.4f' % np.average(test_mrr))
...
with torch.no_grad():
    ...
    for _ in range(10):
        ...
        for ai, bi in zip(ylabel, res.argsort(descending=True)):
            test_res += [ai[bi.cpu().numpy()]]
    test_ndcg = [ndcg_at_k(resi, len(resi)) for resi in test_res]
    oaglog.logger.info('Best Test NDCG: %.4f' % np.average(test_ndcg))
    test_mrr = mean_reciprocal_rank(test_res)
    oaglog.logger.info('Best Test MRR:  %.4f' % np.average(test_mrr))
