...
from DistanceFeature import get_distance_feature
from IdentityFeature import compute_identity
...
# def get_meta_adj(G, meta_graph):
#     """
#
#     :param meta_graph: 元关系三元组，target_type, source_type, relation_type
#     :return: 元关系三元组下的邻接矩阵
#     """
#     target_type, source_type, relation_type = meta_graph
#     meta_edge_dict = G.edge_list[target_type][source_type][relation_type]
            ...
            for relation_type in tes:
                ...
                for target_key in tld:
                    ...
                    for source_key in tesr[target_key]:
                        ...
                        # 索引的二层列表；新的edge_list不记录节点索引，但记录节点进入layer_data的顺序；
                        # 而layer_data的前len(inp)个一定来自inp
...
def to_torch(feature,
             time,
             edge_list,
             graph,
             distance_flags=('sp', "rw"),
             distance_params=(4, 4),
    ...
    # 记录被抽样节点的新索引
    # for type in layer_data:
    #     for ser in layer_data[type][0]:
    #         input_node_new_index += [ser + node_num[type]]
    ...
    if (not distance_flags[0]) and (not distance_flags[1]):
    ...
    if use_identity_feature:
        identity_feature = compute_identity(edge_index, node_num, identity_param)

        node_feature = torch.cat([node_feature, identity_feature], dim=1)
