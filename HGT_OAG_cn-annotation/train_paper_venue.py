...
kernel feature
"""
parser.add_argument("--use_identity_feature",type=bool,default=False,
                    help="use identity feature or not")
parser.add_argument("--identity_param",type=int,default=5,
                    help="the parameter of identity feature")
parser.add_argument("--distance_flags",type=tuple,default=(None,None),
                    help="the distance feature to be used")
parser.add_argument("--distance_params",type=tuple,default=(4,4),
                    help="the params of distance feature")
...
def node_classification_sample(seed, pairs, time_range, batch_size):
    ...
    node_feature, node_type, edge_time, edge_index, edge_type, node_dict, edge_dict = \
            to_torch(feature,
                     ...
                     distance_flags=args.distance_flags,
                     distance_params=args.distance_params)
    '''
...
for epoch in np.arange(args.n_epoch) + 1:
    ...
    '''
    train_data = [job.get(timeout=7200) for job in jobs[:-1]]
    valid_data = jobs[-1].get(timeout=7200)
    pool.close()
    pool.join()
    '''
    ...
    with torch.no_grad():
        ...
        if valid_ndcg > best_val:
            best_val = valid_ndcg
            torch.save(model, os.path.join(args.model_dir, args.task_name + '_' + args.conv_name + "ori_distance"))
            print('UPDATE!!!')
