...
# 返回任意两点间最短路径上所有边特征构造的完整特征，
# max_dis表示两个节点最短路径上所容忍的最大边数，即最大跳数；
# path表示由floyd_warshall构造的任意两点最短路径的边；
# edge_feat为原始边特征；
def gen_edge_input(max_dist, path, edge_feat):
    ...
    for i in range(n):
        for j in range(n):
            ...
            # ij最短路径包含的边数
            num_path = len(path) - 1
            # 此时,path = [i,k_1,...,k_n,j],该循环将ij中所包含的边特征取出作为ij的完整边特征；
